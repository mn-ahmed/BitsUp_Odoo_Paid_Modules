...
class WarrantyRenew(models.TransientModel):
    ...
    def action_view_invoice(self):
        action = self.env.ref('bi_warranty_registration.action_warranty_invoice_tree1').sudo().read()[0]
    ...
    def state_renew(self):
        if self.warranty.renew_no < self.product_id.warranty_renewal_time:
            ...
            if self.warranty.warranty_create_date:
                date_1 = (datetime.strptime(str(self.warranty.warranty_create_date), '%Y-%m-%d').date() + relativedelta(
                    months=+ months_w))
            else:
                date_1 = False
